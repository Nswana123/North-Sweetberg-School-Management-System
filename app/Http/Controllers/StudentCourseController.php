   ...
   public function showCourseRegistrationForm()
{
    ...
    $registeredCourseIds = StudentCourseRegistration::where('student_id', $student->id)
        ->where('academic_year', now()->year)
        ->pluck('course_id')
        ->toArray();

    // Available courses for student's program and year excluding already registered ones
    $availableCourses = DB::table('course_program_year')
        ...
        ->whereNotIn('course_program_year.course_id', $registeredCourseIds)
        ->select('courses.*')
        ->get();
