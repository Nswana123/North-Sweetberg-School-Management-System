...
<div class="modal fade" id="filterModal" tabindex="-1" aria-labelledby="filterModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <form method="GET" action="{{ route('payments.index') }}" class="modal-content">
        ...
        <tbody>
            @forelse($students as $student)
                <tr>
                    ...
                    <td>K{{ number_format($student['total_fees'], 2) }}</td>
                    <td>K{{ number_format($student['total_paid'], 2) }}</td>
                    <td>
                        <span class="badge {{ $student['balance'] > 0 ? 'bg-danger' : 'bg-success' }}">
                            K{{ number_format($student['balance'], 2) }}
                        </span>
                    </td>
                    <td>
                        <span class="badge bg-info">{{ $student['transaction_count'] }}</span>
                    </td>
                    <td>
                        <a href="{{ route('payments.transactions', $student['id']) }}" class="btn btn-sm btn-info">
                            View
                        </a>
                    </td>
                </tr>
